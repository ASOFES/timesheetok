...
class ApiService {
  ...
  // Get Timesheet Resume with Scope - NOUVELLE MÉTHODE POUR L'HISTORIQUE
  static Future<Map<String, dynamic>> getTimesheetResume(int userId, int scope) async {
    try {
      print('📊 Loading timesheet resume for user: $userId, scope: $scope');
      
      // Construire l'URL selon le scope
      final scopeLabel = _getScopeLabel(scope);
      final url = '$baseUrl/Timesheet/Resume/UserId/$userId/scope/$scope';
      ...
      print('📡 Resume response status: ${response.statusCode}');

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        print('✅ Resume loaded successfully for scope $scope ($scopeLabel)');
        return {
          'success': true,
          'data': data,
          'scope': scope,
        ...
        try {
          final errorData = jsonDecode(response.body);
          final message = errorData['message'] ?? 'Failed to load timesheet resume';
          throw Exception('API Error ${response.statusCode}: $message');
        } catch (jsonError) {
          throw Exception('HTTP Error ${response.statusCode}: ${response.body}');
        }
      ...
      print('❌ Get timesheet resume error: $e');
      throw Exception('Failed to load timesheet resume: ${e.toString()}');
    }
  }

  // Helper method to get scope label
  static String _getScopeLabel(int scope) {
    switch (scope) {
      case 1:
        return 'Aujourd\'hui';
      case 2:
